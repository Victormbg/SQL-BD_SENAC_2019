...
INNER JOIN serv_prof  serpro ON pro.codprofissional =  serpro.codprofissional
inner join servico ser on serpro.codservico = ser.codservico where ser.codservico = 234;
/*
13) Mostrar o nome e telefone de todos os clientes que entraram com o carro para serviço no dia 15/01/2019.
*/
SELECT nome, telefone FROM cliente cli 
INNER JOIN veiculo vei ON cli.codcliente = vei.codcliente 
inner join servico ser on vei.placa = ser.placa where ser.data_entrada ='2019/01/15';
/*
...
*/
SELECT vei.placa,especialidade from veiculo vei
inner join servico ser on vei.placa = ser.placa
inner join serv_prof serpro ON ser.codservico =  serpro.codservico
inner join profissional pro on serpro.codprofissional = pro.codprofissional where especialidade ='lanterneiro';
